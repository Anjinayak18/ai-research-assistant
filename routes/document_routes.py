...
async def upload_document(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    document = schemas.DocumentCreate(
        document_id=document_id, document_name=file.filename, file_path=file_path
    )

    saved_document = crud.create_document(db, document)
    ...
    result = pipeline.process(file_path, document_id, file.filename)
    ...
    vector_store.add_documents(result["chunks"], embeddings)
    # -----------------------------
    # Update Database
    # -----------------------------
